...
/*   Updated: 2023/04/12 16:40:01 by aaugu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv, char **envp)
{
	...
	if (open_files(&pipex, argv[1], argv[4]) == ERROR)
		error_exit(&pipex, "Problem while opening files", 1);
	pipex.paths = parse_paths(envp);
	if (!pipex.paths)
		error_exit(&pipex, "Problem while parsing PATH env variable", 1);
	if (pipe(pipex.pipe) == ERROR)
		error_exit(&pipex, "Problem while creating pipe.", 1);
...
int	open_files(t_pipex *pipex, char *file1, char *file2)
{
	...
	pipex->fd_out = open(file2, O_CREAT | O_RDWR | O_TRUNC, 0644);
